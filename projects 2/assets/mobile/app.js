...
;(function(){
  ...
  function buildTripPrompt(){
    ...
    var lines = [];
    lines.push('请为以下行程生成详细的旅行建议，包括每天的天气预测、出行建议和注意事项。');
    lines.push('');
    ...
    seg.forEach(function(s, i){
      ...
      if(s.destination) lines.push('  - 目的地: ' + s.destination);
    ...
    lines.push('');
    lines.push('请以JSON格式返回，格式如下:');
    lines.push('{');
    lines.push('  "daily_tips": [');
    lines.push('    {');
    lines.push('      "date": "YYYY-MM-DD",');
    lines.push('      "weather": "天气描述",');
    lines.push('      "temperature": "温度范围",');
    lines.push('      "tips": "出行建议",');
  ...
  function buildDayCard(day, idx){
    ...
    if(day.weather || day.temperature){
      ...
      if(day.ai_tips) widgets += '<div class="weather-tips">💡 ' + day.ai_tips + '</div>';
